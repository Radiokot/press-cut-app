...
@Immutable
class SelectDestinationCollectionDialogViewModel(
    ...
    private val log by lazyLogger("SelectDestinationCollectionDialogVM")

    private val _collections: List<StampCollection> = runBlocking {
        getSortedStampCollectionsUseCase()
            .first()
            .run {
                if (parameters.request !is SelectDestinationCollectionRequest.MoveStamps) {
                    return@run this
                }

                mapNotNull { collection ->
                    if (collection.id == parameters.request.currentCollectionId) {
    ...
    val message: SelectDestinationCollectionDialogMessage =
        when (val request = parameters.request) {
            is SelectDestinationCollectionRequest.MoveStamps
                if request.isSingleStamp ->
                SelectDestinationCollectionDialogMessage.MoveStamp

            is SelectDestinationCollectionRequest.MoveStamps ->
                SelectDestinationCollectionDialogMessage.MoveStamps

            SelectDestinationCollectionRequest.SaveStamps ->
    ...
    data class Parameters(
        val request: SelectDestinationCollectionRequest,
    )
}
